...
largest_sd_province = horses_sd_table['Province'].values[0]
```
...
for each Province over the time period visualised in @fig-all-provinces 
...
Additionally, note that in @tbl-horses-sd we consider the sample standard deviation of the number of horses during the same time span as @fig-all-provinces.
...
```{python}
#| echo: false
display(Markdown(f"In @fig-largest-sd we zoom in and look at the province of {largest_sd_province}, which had the largest spread of values in terms of standard deviation."))
```
